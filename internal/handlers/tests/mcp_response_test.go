...
func TestMCPToolResponse(t *testing.T) {
	...
	testCases := []struct {
		name      string
		handler   func() (handlers.Handler, map[string]interface{})
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			case *handlers.DockerHandler:
				result, err = h.GetLatestVersion(context.Background(), args)
			case *handlers.PythonHandler:
				result, err = h.GetLatestVersionFromRequirements(context.Background(), args)
			case *handlers.NpmHandler:
				result, err = h.GetLatestVersion(context.Background(), args)
			default:
				t.Fatalf("Unknown handler type: %T", handler)
			}
